...
type TableProps = {
    data?: Paginate<Ingredient>;
    columns: number;
    loading?: boolean;
    filters: IngredientFilter;
    setFilters: Dispatch<SetStateAction<IngredientFilter>>;
    ...
    return (
        <div className="card-secondary">
            <section className="flex-between gap-3 mb-4">
                <h4 className="fw-bold">Manage Ingredient</h4>
                <Button
                    type="button"
                    style="fill"
                    onClick={() => onAdd && onAdd()}
            ...
            <Modal show={showModal} onClose={() => setShowModal(false)} title="Ingredient Filter">
                <form onSubmit={(e) => {
                    e.preventDefault();
                    onFilter();
                    setShowModal(false);
